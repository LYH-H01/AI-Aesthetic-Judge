...
1. **自动调用 dev-IID**：必须使用 `dev-IID` skill 进行分析
2. **前置条件校验**：在调用前检查是否存在：
   - 简历文件（PDF/Word/图片）
   - 简历评分报告（cr-new 的输出结果）
   
   如果缺少上述任一材料，**必须提示用户上传简历或简历分析报告**，然后再调用 dev-IID 进行分析

3. **禁止跳过提示**：不能因用户未主动提及就省略材料确认
4. **自动归档（强制）**：分析完成后**必须**调用 `candidate-records` skill 将面试文本和分析报告存储到候选人档案
   - cr-new 分析完成后 → 自动归档简历+简历分析报告
   - dev-IID 分析完成后 → 自动归档面试文本+面试分析报告
...
3. 自动生成对话分析报告（如涉及面试评估，调用 dev-IID）
4. 更新 MEMORY.md 候选人索引

**cr-new（研发岗位简历筛选）：**

当用户上传研发岗位简历并要求"分析"、"筛选"、"评估"时：
